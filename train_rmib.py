...
if __name__ == "__main__":
    ...
    for epoch in range(1, epochs+1):
        ...
        for i in tqdm(range(num_batch), desc=f"Epoch {epoch}/{epochs}"):
            ...
            loss_gp = F.l1_loss(gp_preds, gp[:, 1:target_frame+1, :])
            loss_c = F.l1_loss(c_preds, c[:, 1:target_frame+1, :])

            # update
            generator_optimizer.zero_grad()
            loss = loss_lq + loss_gvr + 0.5 * loss_gp + 0.1 * loss_c
            loss.backward()
            loss_sum += loss.item()
